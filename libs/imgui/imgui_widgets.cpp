...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, const bool* touched)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if(touched && *touched) { col = GetColorU32(ImGuiCol_ButtonActive); }
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, const bool* touched)
{
    return ButtonEx(label, size_arg, flags, touched);
}
